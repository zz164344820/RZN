package com.rzn.module_driver.ui.bean;

import java.io.Serializable;
import java.util.List;

import mlxy.utils.S;

/**
 * Created by 17662 on 2018/5/27.
 */

public class DriverIdentBean implements Serializable{

    private String belongs;//":"北京善天使合作社",
    private String birthday;//":"1990-11-12",
    private String birthdayAfter;//":"90后",
    private String carNo;//":"京N:8888",
    private String carPic1;//":"http://localhost:80/farmHand/handler/downloadFile?fileName=icon_1521710439364_718.jpg",
    private String carPic2;//":"",
    private String carType;//":"大众POLO",
    private String createTime;//":"2018-03-21 17:11:38",
    private String endDate;//":"2018-03-20",
    private String handlerId;//":"40289f6c6247d1a4016247d400d40001",
    private List<DriverTypeBean> handlerKindTypeArray;//"
    private String icon;//":"http://localhost:80/farmHand/handler/downloadFile?fileName=default.jpg",
    private String idNo;//":"340403199011103456",
    private String isJoin;//":"1",
    private String latitude;//":"116.324615",
    private String level;//":"5",
    private String longitude;//":"39.736762",
    private String machinePic1;//":"http://localhost:80/farmHand/handler/downloadFile?fileName=machinePic_1521707183116_694.jpg",
    private String machinePic2;//":"",
    private String mobile;//":"18880808080",
    private String name;//":"胡机手1",
    private String sex;//":"1",
    private String startDate;//":"2018-03-10",
    private String status;//":"3",
    private String updateTime;//":"2018-05-12 17:13:39",
    private String userId;//":"40288ad75c81124b015c8132bfe8006f",
    private String years;//":"10"

    public String getBelongs() {
        return belongs;
    }

    public void setBelongs(String belongs) {
        this.belongs = belongs;
    }

    public String getBirthday() {
        return birthday;
    }

    public void setBirthday(String birthday) {
        this.birthday = birthday;
    }

    public String getBirthdayAfter() {
        return birthdayAfter;
    }

    public void setBirthdayAfter(String birthdayAfter) {
        this.birthdayAfter = birthdayAfter;
    }

    public String getCarNo() {
        return carNo;
    }

    public void setCarNo(String carNo) {
        this.carNo = carNo;
    }

    public String getCarPic1() {
        return carPic1;
    }

    public void setCarPic1(String carPic1) {
        this.carPic1 = carPic1;
    }

    public String getCarPic2() {
        return carPic2;
    }

    public void setCarPic2(String carPic2) {
        this.carPic2 = carPic2;
    }

    public String getCarType() {
        return carType;
    }

    public void setCarType(String carType) {
        this.carType = carType;
    }

    public String getCreateTime() {
        return createTime;
    }

    public void setCreateTime(String createTime) {
        this.createTime = createTime;
    }

    public String getEndDate() {
        return endDate;
    }

    public void setEndDate(String endDate) {
        this.endDate = endDate;
    }

    public String getHandlerId() {
        return handlerId;
    }

    public void setHandlerId(String handlerId) {
        this.handlerId = handlerId;
    }

    public List<DriverTypeBean> getHandlerKindTypeArray() {
        return handlerKindTypeArray;
    }

    public void setHandlerKindTypeArray(List<DriverTypeBean> handlerKindTypeArray) {
        this.handlerKindTypeArray = handlerKindTypeArray;
    }

    public String getIcon() {
        return icon;
    }

    public void setIcon(String icon) {
        this.icon = icon;
    }

    public String getIdNo() {
        return idNo;
    }

    public void setIdNo(String idNo) {
        this.idNo = idNo;
    }

    public String getIsJoin() {
        return isJoin;
    }

    public void setIsJoin(String isJoin) {
        this.isJoin = isJoin;
    }

    public String getLatitude() {
        return latitude;
    }

    public void setLatitude(String latitude) {
        this.latitude = latitude;
    }

    public String getLevel() {
        return level;
    }

    public void setLevel(String level) {
        this.level = level;
    }

    public String getLongitude() {
        return longitude;
    }

    public void setLongitude(String longitude) {
        this.longitude = longitude;
    }

    public String getMachinePic1() {
        return machinePic1;
    }

    public void setMachinePic1(String machinePic1) {
        this.machinePic1 = machinePic1;
    }

    public String getMachinePic2() {
        return machinePic2;
    }

    public void setMachinePic2(String machinePic2) {
        this.machinePic2 = machinePic2;
    }

    public String getMobile() {
        return mobile;
    }

    public void setMobile(String mobile) {
        this.mobile = mobile;
    }

    public String getName() {
        return name;
    }

    public void setName(String name) {
        this.name = name;
    }

    public String getSex() {
        return sex;
    }

    public void setSex(String sex) {
        this.sex = sex;
    }

    public String getStartDate() {
        return startDate;
    }

    public void setStartDate(String startDate) {
        this.startDate = startDate;
    }

    public String getStatus() {
        return status;
    }

    public void setStatus(String status) {
        this.status = status;
    }

    public String getUpdateTime() {
        return updateTime;
    }

    public void setUpdateTime(String updateTime) {
        this.updateTime = updateTime;
    }

    public String getUserId() {
        return userId;
    }

    public void setUserId(String userId) {
        this.userId = userId;
    }

    public String getYears() {
        return years;
    }

    public void setYears(String years) {
        this.years = years;
    }

    @Override
    public String toString() {
        return "DriverIdentBean{" +
                "belongs='" + belongs + '\'' +
                ", birthday='" + birthday + '\'' +
                ", birthdayAfter='" + birthdayAfter + '\'' +
                ", carNo='" + carNo + '\'' +
                ", carPic1='" + carPic1 + '\'' +
                ", carPic2='" + carPic2 + '\'' +
                ", carType='" + carType + '\'' +
                ", createTime='" + createTime + '\'' +
                ", endDate='" + endDate + '\'' +
                ", handlerId='" + handlerId + '\'' +
                ", handlerKindTypeArray=" + handlerKindTypeArray +
                ", icon='" + icon + '\'' +
                ", idNo='" + idNo + '\'' +
                ", isJoin='" + isJoin + '\'' +
                ", latitude='" + latitude + '\'' +
                ", level='" + level + '\'' +
                ", longitude='" + longitude + '\'' +
                ", machinePic1='" + machinePic1 + '\'' +
                ", machinePic2='" + machinePic2 + '\'' +
                ", mobile='" + mobile + '\'' +
                ", name='" + name + '\'' +
                ", sex='" + sex + '\'' +
                ", startDate='" + startDate + '\'' +
                ", status='" + status + '\'' +
                ", updateTime='" + updateTime + '\'' +
                ", userId='" + userId + '\'' +
                ", years='" + years + '\'' +
                '}';
    }
}
